...
export class Status extends Field
{
    ...
    translate(gtg_value=null, dav_value=null)
    {
        for (var gtg_status, dav_status in this.status_mapping) {
    ...
    {
        return this.translate(dav_value=super().get_dav(todo, vtodo))[1];
    }

    write_gtg(task: Task, value, namespace: string = undefined)
    {
        value = this.translate(dav_value=value, gtg_value=value)[0];
